        ...
        if filename.endswith('.html'):
            ...
            with open(filepath, 'r', encoding='utf-8') as f:
                ...
                for link in soup.find_all('a', href=True):
                    ...
                    if link_url.startswith('https://dodi-repacks.site'):
...
async def search(interaction: discord.Interaction, input: str):
    ...
    if not results_elamigos and not results_steamrip and not results_dodi:
        await interaction.followup.send('Oops! I couldnt find any results :(')
        return
    ...
    embed = discord.Embed(title=f"🔎 Results for '{input}'", color=discord.Color.dark_gray())
    if results_elamigos:
        embed.add_field(name=f'{elamigos_emoji} ELAMIGOS', value='\n'.join(results_elamigos[:5]), inline=False)
    if results_steamrip:
        embed.add_field(name=f'{steamrip_emoji} STEAMRIP', value='\n'.join(results_steamrip[:5]), inline=False)
    if results_dodi:
        embed.add_field(name=f'{dodi_emoji} DODI REPACKS', value='\n'.join(results_dodi[:5]), inline=False)
...
async def games(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title='Cracked Games Websites', color=discord.Color.dark_gray())
    embed.add_field( # DDL
...
async def emulators(interaction: discord.Interaction):
    ...
    embed = discord.Embed(title='Emulators and Roms Popular Websites', color=discord.Color.dark_gray())
    embed.add_field(
        name='Multisystem',
    ...
    embed.add_field(
        name='Microsoft',
        value='- [Xenia](https://xenia.jp/) - Xbox 360',
        inline=False
    )
    embed.add_field(
        name='Sony',
...
@bot.tree.command(name='softwares', description='A selection of good websites for downloading cracked softwares.')
...
@bot.tree.command(name='books', description='Popular websites for downloading and reading books.')
async def books(interaction: discord.Interaction):
    await interaction.response.defer()

    embed = discord.Embed(title='Free Books for Download', color=discord.Color.dark_gray())
    embed.add_field(
    ...
    embed.add_field(
        name='IMPORTANT NOTE',
        value='Try also searching for `<bookname> PDF`'
    )

    await interaction.followup.send(embed=embed)

@bot.tree.command(name='media', description='Websites for watching online movies for free.')
